...
- [x] 시작일 확정 — Day 1 = 2026-08-12(수), ~~Day 20 = 8/31(월)~~ ~~9/3(목)~~ → **Day 20 = 9/8(화) 런칭·게시**, 판정일 9/22(화), Day 6 = 8/20(목), 버퍼 5일 = 9/1~9/5 (8/20 조정 4회차 — AI Summit 8/19 하루 참가로 순손실 0, PLAN §일정 매핑)
...
> **Day 6 착수 메모 (8/20)** — 브랜치 `feat/day6-scr001-home`. 선행: `npx shadcn add input`. ⛔ **순환 의존 발견**: `EL-HOME-003`의 히어로 이미지는 "0번 카드 정적 이미지"인데, 카드 컴포넌트(SCR-003)를 만들어 렌더해야 나온다 → **홈이 카드에 의존**. 해소안 2가지 — ①히어로는 고정 크기 자리만 잡고 진행, 이미지는 SCR-003 이후 교체(PRD 원안 유지) ②카드 컴포넌트를 Day 6 앞으로 당김(카드는 SCR-003·004·라이브러리·OG 4곳 공용이라 최다 재사용 부품). **미결정 — 세션 시작 시 판정할 것**
>
...
### 위생 항목 (SCR 밖 — Day 6~11 = 8/20~8/25 내)
