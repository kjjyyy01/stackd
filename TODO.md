...
> **작업 순서**: [1] 더미 데이터로 카드 컴포넌트 — **단계 8 + 태그 4 상한 케이스**를 2~3개 비율로 렌더 → [2] 본인이 보고 **OQ-006(세로형 비율)·OQ-010(상세 암시)** 판정 → [3] 0번 카드 콘텐츠 작성 → [4] 히어로 이미지 export → [5] SCR-001 나머지(EL-HOME-001·002·004~015).
> ⚠️ 잔여: `EL-CARD-014`(돌아가서 수정 링크) 라벨이 **CPY 미정** — SCR-003 구현 시 `11_UX카피사전.md`에 신규 ID 필요
>
