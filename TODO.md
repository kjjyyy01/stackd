...
- [x] 카톡/X 미리보기 육안 확인 · 비공개 타인 404 확인 ← **8/26 프로덕션 실측**(main 머지·배포 후). **카카오톡 미리보기 정상**(본인 확인) — 카드 제목·단계가 그려진 동적 OG. 비공개 전환 시: 타인 상세 **404**(BR-006) · 사이트맵에서 제외 · `/api/og` 새 `v`는 **기본 이미지**(37,518b, 동적은 45,353b). 기존 `v`는 `x-vercel-cache: HIT`으로 캐시 잔존 — **OPS-001 §11 #5·PRD-08 대응표 #2에 기록된 기존 한계**(실제 `togglePublic`은 `updated_at`을 갱신해 `v`가 바뀌므로 새 URL은 기본 이미지). ⚠️ 잔여: **X(트위터) 미리보기 미확인**
