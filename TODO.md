...
- [x] 시작일 확정 — Day 1 = 2026-08-12(수), ~~Day 20 = 8/31(월)~~ ~~9/3(목)~~ → **Day 20 배포 9/8(화) 완료 · 런칭·게시 = 9/10(목)**, 판정일 9/24(목), Day 6 = 8/23(일), 버퍼 2일 = 9/4~9/5 (8/22 버퍼 소진 2 — 8/21~22 이틀이 산출물 0건으로 지나가 지렛대 0으로 2일 추가 흡수, 누적 3일, 런칭일 불변, PLAN §일정 매핑)
...
- [x] ~~404~~(8/18 완료) / ~~robots(noindex 규칙)~~ / ~~정적 기본 OG~~ / ~~sitemap 동적~~ ← **8/26** `app/robots.ts`(disallow 5개 — `/card$`는 **끝 앵커 필수**, 없으면 접두사 매칭이 `/card-detail/{id}`까지 막아 H-02 공유 루프가 죽는다) · `app/sitemap.ts`(정적 4 + 공개·!hidden 상세 동적, `updated_at` lastmod — hidden은 RLS를 통과하므로 앱에서 필터) · `app/opengraph-image.tsx`(1200×630 빌드타임 생성). **비율 결함 1건 수정**: 홈·상세가 OG로 쓰던 `hero-card.png`는 1120×1400(4:5)이라 카톡·X에서 잘렸다 → 지정 제거하고 기본 OG 상속. 실측: 전 페이지 og:image 1200×630 상속, robots.txt·sitemap.xml 200. ~~잔여 [본인] favicon 교체 예정~~ → **8/28 완료**(`22d4886` — `app/icon.png` 32×32 · `app/apple-icon.png` 180×180, Next 기본 `favicon.ico` 제거. 메모만 남아 9/2까지 미결로 보였다) · ~~기본 OG 한글 미포함(OQ-003 폰트 판정 대기)~~ → **9/2 해소**(`181d59e` — 태그라인 "내 AI 워크플로우를 카드 한 장으로 — stackd.kr", 그 문장 글자만 서브셋한 woff 28.9KB를 `lib/og-font.ts`에 base64 내장. 빌드 타임 정적 OG는 fetch가 불가하고 서버리스는 `public/`을 못 읽는다) · ~~`public/hero-card.png` 참조 0건~~ → **8/26 제거**(Next 보일러플레이트 svg 5종 동반 제거, 사용자 지시)
...
- [x] Day 13: 남은 폴리싱 — ~~방향성 전환~~(8/30 좌우 슬라이드 완료) · ~~stagger 검토~~(8/30 컷 권고, 위 #5) · **GSAP 지식 총정리 9/1 EOD 세션에서 종결** — scratchpad 초안은 세션 디렉터리와 함께 소실됐으나 원자료가 `ANIMATION.md`에 남아 있었다. **총정리를 새로 쓰지 않았다**: GSAP 지식은 이미 TIL 3건(reduced-motion 킬스위치·이징 지수·scroll-behavior 뒤로가기)으로 존재해 요약본을 만들면 링크 그래프가 갈라진다(EOD 규칙 5). 기존 3건 어디에도 없던 **기각 6건**(`ANIMATION.md:54` — 빈도·동선·레이아웃 영향·콘텐츠 성격·중복·우선순위 6개 판정 축)만 뽑아 TIL 1건으로 종결

### Day 15 (9/1) — 출시 준비 ①: 관측성 (PLAN Day 15~16 블록)

- [x] **Sentry SDK 연동** ← `@sentry/nextjs@10.72.0`(peer `next: ^16.0.0-0` 확인). **파일 3개**: `instrumentation.ts`(서버 `register` + `onRequestError = Sentry.captureRequestError`) · `instrumentation-client.ts`(하이드레이션 직전 init + `onRouterTransitionStart`) · `next.config.ts`(`withSentryConfig`는 `SENTRY_AUTH_TOKEN` 있을 때만 감싼다 — 자격증명 없는 환경에서 빌드가 깨지지 않게). **`sentry.edge.config.ts`는 만들지 않았다** — Next 16 proxy는 Node 런타임 기본이고 `runtime` 지정 자체가 에러(`next/dist/docs/…/proxy.md:223`), 앱 전체 `runtime="edge"` 0건이라 대상이 없다. **DSN 없으면 init 스킵** — `components/analytics.tsx`의 GA4 규칙과 동일(로컬·프리뷰 오염 방지). **PII 금지**(PRD-14): `sendDefaultPii: false` + **세션 리플레이 미도입**(화면 녹화는 개인정보 처리라 처리방침 개정이 선행) + `tracesSampleRate: 0`(에러만, 무료 쿼터 보존). env 4종 추가 → PRD-17 표·`.env.example` 반영. ⚠️ **번들 실측 +221,865 raw / +69,533 gz (+22.0%)** — `.next` 삭제 후 2회 대조(오탐 1건 배제: 청크의 `sentry-mcp` 문자열은 `data/catalog.json` 데이터였다). Sentry의 트리셰이킹 define(`__SENTRY_TRACING__`)은 **webpack 경로에만 구현**(`config/webpack.js:565`)돼 Turbopack 빌드에서는 줄일 수단이 없다 → **Day 17 LCP 측정에서 재판정**, 예산 초과 시 지렛대 = 클라이언트 SDK 제거(서버 전용으로 −68KB). ~~DSN 발급·env 등록~~ ✅ 9/1 완료(US 리전) — **Vercel env 정리까지**: `SENTRY_AUTH_TOKEN`은 `Sensitive`+**Production 전용**(기존 시크릿 7종과 기준 일치), `DSN`·`ORG`·`PROJECT`는 Non-sensitive+양쪽(공개 값이라 맞다). ~~잔여 [본인]~~ ✅ **9/1 연동 완료 판정**: 기본 알림 규칙(`Send a notification for high priority issues` — Sentry 자동 생성, `Created by: —`)이 **KST 16:00에 발화**했고(`Total Triggers: 1` — 14일 창에서 유일, 임시 라우트 호출 시각과 일치) **이메일 실수신 확인(본인)**. 즉 "SDK가 켜졌다"가 아니라 **"에러가 나면 사람이 알게 된다"**까지 실증. 설정은 **손대지 않았다** — 발화량 1회/14일이라 환경 필터(`vercel-production` 한정)는 조이면 프리뷰 검증 구간만 눈멀게 해서 보류, WHEN이 `high priority`로 한정된 구멍은 PLAN 런칭 후 루틴의 "매일 sentry 신규 에러 확인"이 메운다(알림=긴급 통로 / 데일리=전수 통로 2겹). ⚠️ **조직에 다른 프로젝트 1개 존재** — 무료 쿼터 5천 건/월은 **조직 단위 공유**라 잔재면 정리 권고
...
- [x] **장애 대비 3종 문서화** ← `docs/runbook.md`(54줄). 롤백 명령(`vercel rollback` — CLI 59.4.0 실측) · 백업 절차(Supabase Free는 자동 백업 없음 → CSV 수동) · 런칭일 행동 순서 3줄 + 이미 겪은 실패 모드 5건. **cascade 경고 명시**: `supabase/schema.sql:11` `on delete cascade` — auth.users 삭제 시 그 사용자의 workflows 전멸이라 **런칭 카드 5장이 탈퇴 테스트로 날아갈 수 있다**. `feedback.workflow_id`는 `on delete set null`이라 피드백은 생존(`:42`). ~~잔여 [본인]~~ ✅ **9/1 롤백 리허설 1회 완료** — `vercel rollback` → `stackd.kr` 정상 → `vercel promote` 복귀, 4단계 전부 정상(1분 미만). **런북에 `promote` 누락이 리허설로 드러나 보강** — "되돌리는 법"만 있고 "돌아오는 법"이 없으면 리허설 자체가 사고가 된다. ✅ **9/1 CSV 백업 1회 완료** — `~/backups/stackd/2026-09-01/`에 `workflows_rows.csv`(6행/16컬럼)·`feedback_rows.csv`(3행/7컬럼). repo 유입 0건(`git ls-files '*.csv'` 비어 있음). ⚠️ **행 수는 `wc -l`로 세면 틀린다** — `situation`·`steps.detail`이 pre-wrap 다행 텍스트라 셀 안에 개행이 있다(csv 파서로 셀 것). **장애 대비 3종 전부 종료**(문서·리허설·백업)
...
- [x] **네이버 서치어드바이저 등록·사이트맵 제출**(본인, 2026-09-08) ← 소유확인은 **HTML 태그**(`layout.tsx` `metadata.verification.other`, `962d24b` 프로덕션 실측) — 화면에 DNS 옵션 없음. 사이트맵 `sitemap.xml` 제출 + 웹 페이지 수집 3건(홈·라이브러리·934c9228). 네이버는 Google과 별개 색인, 노출까지 약 2주. **메타태그 지우면 재인증**(유효 1년, 만료 30일 전 메일)
...
- [x] **`steps[].tool.custom` 신설 + 빌더 체크박스 + 카드·상세·OG 3표면 배지 + 문서 동기화** ← `feat/custom-tool-toggle` 브랜치, 테스트 39개 통과·lint 0·tsc 0·build 통과, 최종 전체 리뷰 Approved. **머지 완료(2026-09-06, main)** — 프리뷰 3표면 시각 확인 통과, Safari 캡처만 권한 부재로 육안 확인 대체
- 검색 키워드 검수는 **조건 미충족으로 스킵** — Day 1 유입 전략이 "입소문·공유 주채널, 검색은 기본 메타데이터 수준으로 경량화"(`docs/day1-decisions.md:50`)

### 버퍼 (9/6) — 라이브러리 카드 겹침 수정 + 페이지네이션 UI 교체

- [x] **`/workflows` xl 카드 겹침 수정** ← `xl:[--s:0.6]`이 3열 튜닝값(`lg:0.52`)을 덮어써 셀보다 37.3px 넓어진 게 원인. 삭제로 해결, 4개 뷰포트 실측 완료(`c6ca3e5`)
- [x] **gap-8→gap-4 + 카드 확대** ← 컬럼폭 실측 기반 `--s` 재계산(`115583d`)
- [x] **"더 보기" → shadcn Pagination 교체** ← `pageCount`·`pageWindow` TDD 추가, PRD SCR-006 v1.1.0 갱신. **부수 발견·수정**: `count: "exact"` + 범위 초과 `range()` 동시 사용 시 PostgREST 416(PGRST103) — 프로덕션 실사용 조건(6건, `?page=2`)에서 바로 재현되던 버그, count 쿼리 분리로 해결

### Day 18~19 (9/6~9/7) — 런칭 포스트·채널 준비

- [x] **채널 축소 판정** ← 원안 3곳(GeekNews·디스콰이엇·Threads) → **Threads 단독**(2026-09-07). GeekNews는 가입 후 1주 대기, 디스콰이엇은 제품 승인 필요로 런칭 당일 게시 불가. 조건 충족 시 순차 합류
- [x] **포스트 초안 3종 + 제목 확정** ← 게시판 2곳 모두 2번 후보(문제 제기형). Threads는 실게시물 작성창에서 본인 목소리로 리라이팅 완료, `docs/launch-posts.md` 본문은 초안 기록으로 보존
- [x] **전 링크 UTM 검증** ← 링크 6종·12곳이 PRD `15_분석및추적.md` §UTM 규격과 일치, 카드 id `934c9228` 일관. 미태깅 0건
- [x] **Threads OG 미리보기 실측**(본인) ← `og:title`·이미지 정상 렌더링
- [ ] **GeekNews·디스콰이엇 OG 미리보기 실측**(본인) — 게시 가능 시점에 별도 확인 (`docs/launch-posts.md:129`)

### Day 20 (9/8) — 최종 배포 + 보안·SEO 점검

- [x] **프로덕션 최종 배포·검증** ← `be689d1`
- [x] **보안 점검 7종 + 모의 침투** ← gitleaks·OSV·semgrep·설치 스크립트·IDOR·forced-browsing·SSRF·JWT 전부 PASS/N-A. PT-01(보안 헤더 부재)·PT-02(`x-powered-by`) 조치 배포(`2a32426`). CSP는 GSAP·GA4·Supabase 파손 위험으로 런칭 당일 의도적 제외
- [x] **SEO 점검·조치** ← `claude-seo audit` 90/100 → BreadcrumbList·ItemList·sitemap lastmod 3건 조치 후 100. "GSC 미등록" High는 오탐(Day 17에 등록 완료)
- [x] **한글 브랜드명 노출** ← title 3곳 "Stackd(스택드)", `WebSite.alternateName` + `Organization` JSON-LD(sameAs: GitHub·Threads). 영문 "stackd"는 글로벌 경합어라 목표를 "스택드"·"stackd.kr"로 조정
- [x] **런칭일 통일** ← 배포는 9/8 완료, **공개 런칭·게시 9/10(목)**, 판정일 9/24(목)

### Day 21 (9/9) — 계획 변경 + 런칭 준비 문서

- [x] **Day 21 계획 변경** ← ~~초기 유저 3~5명 인터뷰 섭외~~ → **본인 직접 테스트로 대체**(사용자 판정). PLAN.md 4곳 갱신. ⚠️ 사용성 검증("처음 보는 사람이 어디서 멈추는가")은 대체되지 않음을 문서에 명시, 자발적 피드백 전건 기록을 운영 루틴에 추가
- [x] **`docs/day21-self-test.md`** ← 실사용 경로 순회 시나리오 A~E 20항목 + 마찰 기록 템플릿 + 심각도 3단계(치명=즉시 수정 / 마찰·사소=backlog)
- [x] **`docs/kill-criteria-verdict.md`** ← 9/24 판정 시트. 기준 3종·보조 지표 5종·판정 4택. **기준 ③(표본 300명) 미달이면 ①②를 계산하지 않는다**
- [x] **판정 날짜 표기 정정** ← "(2026-09-10 사용자 판정)"이 판정일보다 미래여서 성립 불가 → 2026-09-08로 정정, PLAN.md 2곳·`launch-posts.md` 2곳(한 줄에 2회 등장)
- [x] **Day 10~20 EOD 소급 기록** ← EOD 커버리지 감사에서 발견한 미기록 구간. Day 10·18·19·20·버퍼일(9/5)은 신규 작성, Day 11·12~13·14는 기존 기록 완비 확인
- [ ] **런칭 D-1 프로덕션 헬스체크** — 주요 URL·OG·sitemap·robots·분석 스크립트 존재 확인
- [ ] **PLAN.md 331행 "런칭 직후 점검" 체크 표기** — 보안·SEO는 9/8에 선행 수행됨. 완료 판정은 [본인]

### 런칭 (9/10) 이후 — 전부 [본인] 실행

- [ ] **Threads 게시** — 1번 글(이미지+훅) → 2번 글(링크). 3번 글은 재게시용으로 보류 확정. 첨부는 0번 카드 PNG 또는 히어로 GIF
- [ ] **본인 직접 테스트 순회** — `docs/day21-self-test.md` 기준, 프로덕션에서 모바일 1회·데스크톱 1회. 순회 중 수정 금지
- [ ] **자발적 피드백 전건 기록** — 댓글·문의 폼·Threads 반응. 외부 사용자 신호는 이제 이 경로에서만 들어온다
- [ ] **운영 루틴 2주** — 매일 30분 대응(댓글·Sentry·치명 버그만) / 지표는 주 2회 / **신기능 개발 금지**(판정 오염)
- [ ] **9/24(목) 킬 크라이테리아 판정** — `docs/kill-criteria-verdict.md`에 GA4 수치 기입 → 지속·피벗·중단·보류
