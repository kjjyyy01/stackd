...
- 미검증(사용자 작업 후): 배포 URL 로그인 왕복(Day 5 게이트). 커밋은 사용자 지시 시
...
- **EOD TIL 후보(Day 4)**: ① ORM 도입 판단 기준 = 타입이 아니라 "누가 auth.uid()를 채우느냐"(RLS 자동성) ② Next 16 middleware→proxy 리네임 + `getClaims()` 조기 호출 패턴 ③ 시크릿 경계로 사람/에이전트 역할 분리(U/C 체크리스트) ④ Node 26 네이티브 TS로 테스트 러너 0 의존 ⑤ RLS 자기참조 서브셀렉트가 재귀 에러를 피하는 조건
- 잔여(Claude): C7 로컬 왕복 확인 · C9 카탈로그 검수 · C10 GA4·sonner·shadcn(테마 결정 후) · C11 프리뷰→main
