...
## 2026-08-27 — Day 10 EOD 기록 (Notion 8페이지 / Obsidian TIL 4신규·1보강, 2026-09-09 소급 작성)

### 작업 내용

- **무엇을**: Day 10(8/27) EOD 기록 블록 — 커밋 23개(`94a30b2`~`6b48830`)와 이 문서의 8/27 블록을 소스로 트러블슈팅 7건·과정요약 1건을 이중 기록(PLAN.md §기록 시스템). **런칭 이후 소급 작성**이라 원고 스테이징 후 승인 게이트는 사용자 지시로 생략
- **어떻게**:
  - **등록 전 목적지 조회를 먼저 했다** — Notion「개발 로그:stackd」를 `2026-08-25 ~ 08-31` 범위로 SQL 조회. 그 결과 **Day 11은 이미 4건(과정요약 1 + TS-D11-1~3)이 등록돼 있었고 Day 10만 0건**이었다. 감사에서 "Day 10·11 EOD 없음"으로 잡혔던 것은 이 문서에 "Day N EOD 기록" 헤더가 없다는 근거였는데, 그건 산출물이 아니라 산출물에 **대한** 진술이다
  - **Notion 8페이지** — 과정요약 1 + 트러블슈팅 7(TS-D10-1~7). 태그는 캐노니컬 4종(gsap·성능·기획·배포) 밖으로 나가지 않았고, 추측 대신 미태깅 유지(Day 6 판례). 기획 3건(과정요약·TS-D10-2·TS-D10-6), 나머지 4건 미태깅
  - **Obsidian TIL 신규 4** — 증상만 고친 수정은 다음 라운드에 자기가 만든 구멍을 남긴다 / 한 개념의 판정 원천이 둘이면 화면과 저장이 다른 말을 한다 / RLS 읽기 정책은 소유자 필터가 아니라 가시성 필터다 / 정리 스크립트의 삭제 조건은 생성 경로의 가장 넓은 공통 마커여야 한다
  - **Obsidian 기존 1건 보강** — `TIL-StrictMode-이중-실행은-읽으면-사라지는-값을-깨뜨린다`에 「소비자가 둘이면 StrictMode 없이도 깨진다」 사례 추가. 본문이 "한 이펙트가 두 번 도는" 상황만 다뤄, 형제 컴포넌트 둘이 같은 일회성 값을 나눠 소비하는 8/27 사례가 그 결론 밖에 있었다
- **왜**: Day 10은 **자기 회귀 2건**이 나온 날이다 — 증상 대응 2라운드가 복제본 생성(데이터 손상)과 `/card` 튕김을 새로 심었다. 이 종류는 "무엇을 고쳤나"보다 **"왜 그 수정이 새 결함이 됐나"**가 재사용 지식이라, 라운드 경계를 살려 기록해야 값어치가 남는다

### 작업 결과

- **Notion 8페이지 등록 완료** — 유형 2종·상태 2종·태그 4종 고정 준수, 전건 상태 `완료`
- **제목 온전성 검증** — 생성 응답의 제목이 잘려 보였으나 `length()` 재조회로 56~79자 온전 확인, 단건 `fetch`로 재확인. 9/1·9/2에 이어 **3회 연속 같은 표시 계층 아티팩트**다. 생성 응답을 신뢰하지 말고 저장소를 다시 읽는다는 판례가 세 번 유효했다
- **Obsidian 159 → 164 파일**(TIL 신규 4 + 원고 1), 새로 건 위키링크 11개 전수 실재 확인 — **깨진 링크 0**
- **Day 11은 신규 등록 0건** — 이미 완비. 이 문서에 EOD 헤더만 없었던 것이라 아래에 확인 기록만 추가
- 원고 스테이징: `_notion-원고-stackd-Day10.md`(240줄, Obsidian 볼트)

### 부수 확인

- 볼트가 병행 프로젝트(scored)와 공유돼 있어 8/27~29자 TIL 14건 중 **stackd 소유는 3건뿐**이었다. 파일명·날짜만으로는 소유 프로젝트를 못 가르므로 본문의 프로젝트명으로 판정했다
- Day 10 이월 항목 중 `/privacy`·`/terms` 본인 눈 확인은 Day 11 → Day 12로 재이월된 것이 Day 11 과정요약에 이미 기록돼 있다
...
### 후속 4 — 유저 테스트 방식 변경 (사용자 판정, 8/28)

- **무엇을**: PLAN의 "게릴라 테스터 3~5명"(Day 12 사전 섭외 → Day 14 실시)을 **본인 신규 GitHub 계정 테스트**로 대체
- **왜**: 사용자 결정 — 섭외할 테스터가 없다. Plan Freeze는 "실행 중 실제로 발견된 문제의 반영"을 허용하므로 규정 위반이 아니다
- **결과**:
  - **두 방식이 잡는 게 다르다는 걸 문서에 명시했다.** 새 계정은 **권한·경계 검증**에 오히려 낫다 — 타인 비공개 404(BR-006)·타인 카드 수정/삭제 차단(BR-024)·**로그인한 비-admin의 `/admin` 404**·타인 신고 왕복은 **계정 1개로는 검증 자체가 불가능**했다
  - 특히 세 번째: `TC-ADMIN-001-01`은 **미로그인만** 검증돼 있었다. 로그인 상태의 비-admin이 게이트에 막히는지는 지금까지 한 번도 안 밟아본 경로다 — 실제 보안 경계인데 미검증이었다
  - 대신 **사용성 검증은 구조적으로 대체 못 한다** — 계정을 새로 파도 머릿속 지식은 초기화되지 않는다. 그래서 **실패 조건 9번은 지우지 않고 그대로 뒀고**, 링크 1개·5분·1명이라도 확보되면 그쪽이 우선이라고 적었다
  - 갱신 위치: §일정 매핑 Day 12~13·14 행 / §Day 14 섹션 제목·오전 블록 / §진행 체크리스트 2줄
...
## 2026-08-28 — Day 11 EOD: 신규 등록 0건 (기존 기록 완비 확인, 2026-09-09)

### 작업 내용

- **무엇을**: Day 11 EOD 작성을 착수했다가, 목적지 조회 결과 **이미 완비돼 있어 신규 등록을 하지 않았다**
- **어떻게**: Notion「개발 로그:stackd」 날짜 범위 SQL + Obsidian 볼트 날짜·프로젝트 grep으로 실재 확인
- **왜**: 되돌리기 비싼 외부 쓰기 앞에서는 "무엇이 밀렸는가"의 근거가 목적지의 행이어야 한다. 이 문서의 헤더 부재를 근거로 삼으면 이미 있는 기록을 중복 생성한다

### 작업 결과

- **Notion 기존 4건 확인** — 과정요약 `[Stackd] Day 11 — 전 화면 완성(SCR 8/8), 스코프 컷 0건` / `TS-D11-1`(xl에서 카드가 열을 17px 넘침 — max-w는 border-box) · `TS-D11-2`(dropdown 화살표 무반응 — data-state는 트리거에 붙는다) · `TS-D11-3`(프로덕션 `/admin` 404 — 탈퇴로 uid 재발급, `ADMIN_USER_IDS`는 옛 uid 참조)
- **Obsidian 기존 3건 + 보강 1건 확인** — `TIL-정상-동작을-실패로-읽게-만드는-건-코드가-아니라-계측-지표다`(v4 rotate·Radix pointerdown·배포 판정) / `TIL-삭제된-계정의-JWT는-만료-전까지-계속-통과한다` / `TIL-기능을-지울-때는-코드-grep이-아니라-개인정보-고지까지-따라간다`, 그리고 `TIL-transform-scale은-페인트만-줄이고-레이아웃-폭은-그대로다`에 8/28자 border-box 보강
- **누락 판정: 없음.** Day 11 과정요약이 BR-003 카피 불일치·Preflight 커서·destructive solid·RLS 명시 필터를 이미 본문에 담고 있어 별도 트러블슈팅으로 쪼갤 잔여가 없다
- ⚠️ **감사 결과 정정** — 9/9 EOD 커버리지 감사의 "Day 11 미작성"은 **위양성**이었다. 진척 판정을 이 문서의 메타 기록으로 한 것이 원인. 남은 미작성 Day(12·14·18·19)도 착수 전 목적지 조회로 재확인해야 한다
...
- **결과**:
  ...
  - **#5 컷 권고 → 채택(본인, 8/30)**: 복귀 morph 충돌(도착 슬롯 opacity 0) 우려로 컷을 권고했으나, `spike/grid-stagger` 프리뷰 실확인에서 충돌이 관측되지 않아 채택 — 예측만으로 컷하지 않고 판정용 구현을 띄운 게 유효했다
  - 잔여: GSAP 지식 총정리·EOD 기록은 초안만(게시는 EOD 세션 승인 후)

## 2026-08-30 — 트러블슈팅: 카드 전환 딜레이 = Vercel 함수 리전 (iad1→icn1)

- **증상**: 프로덕션에서 카드 클릭 → 약 1초 무반응 후 morph 시작 (본인 보고). 복귀 경로도 동일
- **원인 규명 순서**: 클릭→`startViewTransition` 호출까지 실측 871~1,371ms → longtask 0건(클라이언트 무죄) → RSC fetch TTFB 361ms인데 curl 전체 TTFB 574~1,402ms → **`x-vercel-id: icn1::iad1`** — 엣지는 인천인데 **함수가 미국 버지니아(Vercel 기본값)**. Supabase는 한국(41ms). 매 동적 렌더가 한국→미국→한국DB→미국→한국 왕복
- **애니메이션은 무죄**: ViewTransition은 새 페이지 도착 후에야 시작하는 구조 — Day 6부터 있던 내비게이션 지연이 morph 때문에 "보이게" 된 것
- **해결**: Vercel 대시보드에서 함수 리전 icn1(서울) 지정(사용자, 비용 0) — **설정은 다음 배포부터 적용**이라 프로덕션 재배포까지 필요했다
- **결과 실측**: 프리뷰 TTFB 574~1,402ms → **133~138ms**, 프로덕션 재배포 후 `icn1::icn1` 확인·워밍 TTFB 197~285ms(콜드 스타트 시 ~1s 잔존은 Hobby 특성). LCP 예산에도 동일 이득

## 2026-08-31 — Day 14: 유저 테스트(권한·경계 4종) + 3축 리뷰 + 치명 5건 수정

- **무엇을**: ①본인 신규 GitHub 계정으로 권한·경계 4종 실검증(사용자) ②3축 리뷰(정확성·복잡도·스펙 준수) ③코드 성숙도·보안 4종 점검 ④리뷰가 낸 데이터 손상·치명 5건 수정
- **어떻게**: 3축을 독립 서브에이전트로 병렬 디스패치 — 축마다 다른 것을 보도록 프롬프트를 분리(버그 / 삭제 대상 / 문서-구현 일치). 권한 검증은 **미로그인분을 curl 상태코드로 선검증**한 뒤 세션이 필요한 4종만 사용자에게 남겨, 사람이 쓰는 시간을 대체 불가능한 곳에만 배정했다
- **왜**: 3축을 나눈 근거는 "코드가 좋아도 스펙과 다르면 버그"다. 실제로 **정확성 축은 치명 0을 냈고 스펙 축만 치명 3을 냈다** — 축을 합쳤으면 못 잡았을 건들
- **결과**:
  - **권한·경계 4종 전부 통과(본인, 8/31)**: 타인 비공개 404(BR-006) · 타인 카드 수정·삭제 차단(BR-024) · **로그인한 비-admin `/admin` 404**(기존 TC 공백이던 케이스) · 신고→admin 숨김 왕복. 미로그인분은 사전 실측(`/admin` 404 · 비공개 404 · 공개 200 · 없는 ID 404)
  - **첫인상 3문**: "카드로 워크플로우 공유하는 서비스"(가치 전달 성공) · "막힌 점 없었다"(핵심 플로우 이탈 0) · 재방문은 "워크플로우를 여러 개 쓰거나 만들기 좋아하는 사람이면 자주" → **치명적 혼란 0건 → PLAN 규칙상 수정 대상 없음**. 단 이는 사용성 검증이 아니라 경계 검증의 부산물 — 실패 조건 9번(외부 1명)은 살아 있다
  - 3축 집계: 정확성 치명0·주요0·경미3(재정렬 목록 index key 등) / 복잡도 치명0(구조적 오버엔지니어링 없음, 약 -37줄·미사용 의존성 2개) / **스펙 준수 치명3·주요7**
  - **보안 4종 전부 통과**: `NEXT_PUBLIC_` 4개 전부 공개 가능값 · RLS 정책 4종 + feedback 전면 차단 · 전 mutation 경로가 `lib/limits.ts` 검증 통과 · `npm audit` 0건 + git 전 이력 시크릿 0건
  - 반응형 375/1440 4화면(홈·갤러리·상세·빌더) 순회 무손상 · lint 0 · test 31 pass · build 통과
  - **수정 5건**(브랜치 `fix/day14-review-critical`): GA4 파라미터 누락(EVT-CARD-001·003) · step_add 중복 발화 억제(EVT-BLDR-001) · 히어로 색 토큰화 · 갤러리 컨테이너 폭 · admin 조회 실패의 Empty 위장
  - **실측이 나이브 픽스를 막은 건**: 하드코딩 `#3f4a56`을 `--muted-foreground`로 바꾸는 게 "토큰 준수"처럼 보이지만, `--chat-bg` 지면에서 **2.79:1로 AA 미달**이었다(현재 값 5.59:1). 값을 유지한 채 `--chat-fg` 토큰으로 승격하는 게 정답 — 토큰 위반의 해법이 항상 기존 토큰 대체는 아니다
  - **GA4를 최우선에 둔 이유**: 코드 결함은 나중에 고쳐도 되지만 이벤트 파라미터는 소급 적용이 안 된다. 런칭(9/8) 전에 안 고치면 판정일(9/22) 지표가 영구 공백
  - **성숙도 체크리스트 8항목 완주(8/31 밤 추가분)**: 미통과였던 2항목을 같은 날 닫았다 — ①DESIGN.md 대조 ②ARCHITECTURE.md 갱신
    - **두 위반의 실체는 코드가 아니라 스펙 공백이었다**: 서브페이지 h1 8곳이 쓰던 `-0.02em`은 표의 display(h1)도 h2도 아닌 값이었는데, 원인은 표에 **"서브페이지 제목" 역할 행이 없어서**였다. 카드 칩 `#f2f3f6`도 고정 팔레트 4색 표에 없던 5번째 색. 둘 다 **표에 행을 추가하고 코드를 그 값으로 맞추는 방향**으로 닫았다 — 낮에 `--chat-fg`를 처리한 것과 같은 패턴이 하루에 세 번 반복됐다
...
## 2026-08-30 — Day 12~13 EOD: 신규 등록 0건 (기존 기록 완비 확인, 2026-09-09)

### 작업 내용

- **무엇을**: Day 12 EOD 작성을 착수했다가, 목적지 조회 결과 **이미 완비돼 있어 신규 등록을 하지 않았다**
- **어떻게**: Notion「개발 로그:stackd」를 `2026-08-29 ~ 09-01` 날짜 범위 SQL로 조회 + Obsidian 볼트를 날짜·소유 프로젝트로 대조
- **왜**: 되돌리기 비싼 외부 쓰기 앞에서는 "무엇이 밀렸는가"의 근거가 목적지의 행이어야 한다. Day 11에서 같은 위양성을 이미 한 번 겪었다

### 작업 결과

- **Notion 기존 5건 확인** — 과정요약 `[Stackd] Day 12~13 — 모션 설계 6건 전부 구현·판정 완료, 전환 지연은 코드가 아니라 리전으로 해결` / `TS-D12-1`(프로덕션 빌드 module-not-found — `react/canary`는 타입 전용인데 import 문으로 켰다) · `TS-D12-2`(reduce 설정인데 모션이 돈다 — CSS 킬스위치가 GSAP 인라인과 view-transition 의사요소를 못 막는다) · `TS-D12-3`(저장 직후 morph 페어링 실패 — `useTransition` 배칭이 이름을 옛 DOM 스냅샷에 못 실었다) · `TS-D13-1`(카드 클릭 후 ~1초 무반응 — Vercel 함수 리전 iad1)
- **Obsidian 기존 4건 확인**(전부 8/30자 stackd 소유) — `TIL-react-canary-타입은-import문으로-켜면-번들러가-죽는다` / `TIL-reduced-motion-킬스위치는-GSAP과-view-transition-의사요소를-못-막는다` / `TIL-useTransition에-라우팅을-묶으면-ViewTransition-이름이-옛-DOM에-못-실린다` / `TIL-클릭-후-전환-무반응은-코드보다-x-vercel-id-함수-리전부터-본다`
- **누락 판정: 없음.** 트러블슈팅 4건이 TIL 4건과 1:1로 대응한다
- Day 13 이월분(GSAP 지식 총정리)은 **Day 15 EOD에서 이미 종결**됐다(`docs/history.md` 2026-09-01 Day 15 EOD 항목, `TODO.md:127` `[~]` → `[x]`)

---

## 2026-08-31 — Day 14 EOD: Obsidian TIL 1신규 (Notion 기존 4건 완비, 2026-09-09)

### 작업 내용

- **무엇을**: Day 14 EOD 점검. Notion은 완비돼 신규 등록 0건, **Obsidian에서 공백 1건을 발견해 TIL 1건만 신규 작성**했다
- **어떻게**: Notion 날짜 범위 SQL로 기존 4건 확인 → 트러블슈팅 3건이 각각 TIL로 존재하는지 개별 대조 → 없는 1건의 원인을 커밋(`a4626a8`)과 `app/admin/page.tsx:72`에서 재확인 후 작성
- **왜**: 트러블슈팅은 Notion에 있어도 **재사용 지식이 볼트에 없으면 다음 프로젝트로 안 넘어간다.** 두 저장소는 역할이 다르므로 한쪽 완비가 다른 쪽 완비를 뜻하지 않는다

### 작업 결과

- **Notion 기존 4건 확인** — 과정요약 `[Stackd] Day 14 — 권한·경계 4종 실검증 통과 + 3축 리뷰로 잡은 치명 6건 당일 전건 수정` / `TS-D14-1`(admin 목록이 "신고 0건"으로 보인다 — 조회 error를 읽지 않은 폴백) · `TS-D14-2`(`card_create`는 찍히는데 분석 디멘션이 빈다) · `TS-D14-3`(하드코딩 색·자간이 토큰 위반으로 잡힌다 — 스펙 표에 그 역할의 행이 없었다)
- **Obsidian 신규 1** — `TIL-error를-구조분해에서-빼면-장애가-빈-목록으로-위장된다`. `const { data } = await query`가 `error`를 안 받아 실패가 `null` → `data ?? []` → **"신고 0건" Empty로 위장**되던 건(`a4626a8`). 3축 리뷰 TIL이 이 건을 결과 목록에 한 줄 언급만 하고 있어 **교훈 자체는 볼트 어디에도 없었다**
- **기존 2건으로 충분 판정** — `TS-D14-2`는 `TIL-킬-크라이테리아의-분자-이벤트는-구현-당일-실발화로-검증한다` §추가 사례(2026-08-31)가 파라미터 누락·GA4 소급 불가를 이미 담고 있고, `TS-D14-3`은 `TIL-토큰-위반의-원인은-코드가-아니라-스펙-공백일-수-있다`가 정면으로 다룬다. 새 파일을 만들면 링크 그래프만 갈라진다
- **역링크 보강 1** — `TIL-정확성-리뷰와-스펙-준수-리뷰는-다른-것을-잡는다`에 "잡힌 치명 3건의 개별 노트" 줄을 추가해 3건을 전부 연결. 축 노트에서 개별 결함으로 내려가는 경로가 없던 것을 메웠다
- 새로 건 위키링크 4개 + 역링크 3개 전수 실재 확인 — **깨진 링크 0**
- ⚠️ **감사 결과 정정** — 9/9 EOD 커버리지 감사의 "Day 12·14 미작성"도 **위양성**이었다. Day 10만이 진짜 누락이었다. 남은 Day 18·19는 착수 전 같은 방식으로 재확인한다
...
## 2026-09-01 — Day 15 후속: GA4 계측 검수 종결 + 소유자 CTA 어트리뷰션 분기

**무엇을**: ①GA4 맞춤 측정기준 4 + 측정항목 1 등록(본인) ②DebugView 7종 실발화 확인(본인) ③검증 중 발견한 소유자 CTA 결함 수정 — 상세 페이지 CTA가 소유자에게도 UTM과 "나도" 문구를 그대로 노출

**어떻게**: `app/card-detail/[id]/page.tsx`의 `isOwner`(이미 같은 스코프에 있음)로 링크와 문구를 삼항 분기했다. 소유자는 `/`(UTM 없음) + 신설 CPY-WF-016 "카드 하나 더 만들기", 타인·비로그인은 기존 CPY-WF-001 + UTM 그대로. **숨기지 않고 계측만 껐다** — `site-header.tsx`에 생성 진입점이 없어(로고·`/workflows`뿐) CTA를 없애면 소유자의 유일한 생성 경로가 로고 클릭이 된다. hidden 블러 분기(`:75`)는 `wf.hidden && !isOwner` 조건이라 소유자가 도달할 수 없어 손대지 않았다.

**왜**: 이건 **스펙 위반이 아니라 스펙의 빈칸**이었다 — SCR-004 EL-WF-007은 필수 Y에 조건 열이 비어 있어 구현이 정확히 따랐다. 그런데 두 가지가 걸린다. ①**문구**: "나도"는 남의 카드를 볼 때 성립하는 말이라 자기 카드에선 자기 자신을 가리킨다. 같은 파일 EL-WF-001에서 "이미 상세에 도착했으므로 자기 자신을 가리키는 문구가 된다"는 이유로 유도 문구를 뺀 2026-08-23 판정과 **정확히 같은 유형**이다. ②**어트리뷰션**: PRD-15 §UTM의 "내부 링크 UTM은 세션 어트리뷰션을 리셋하지만 의도된 동작"은 *카드 경유 재진입 = 확산*을 계수하려는 것인데, 소유자가 자기 카드에서 누르는 건 확산이 아니다. 계수하면 보조 지표 "루프 작동"(`session_source=card` 세션 수)과 바이럴 계수 k가 부풀려진다. 지금 본인 트래픽은 내부 제외 필터에 걸려 안전하지만, 런칭 후 **다른 소유자들**에게 그대로 발생한다.

**결과**:
- **GA4 등록 완료** — `method`·`item_category`·`share_method`·`is_public`(이벤트 범위) + `step_count`(측정항목, 표준). 우려했던 `item_category` 예약어 충돌은 없었다. **등록은 소급 적용되지 않으므로** 9/22 판정에 쓸 파라미터 데이터의 시계는 이 시점부터 돈다
- **DebugView 7종 실발화 확인** — Happy Path 1회 완주. PRD-15 §실장 검수 절차 2단계 종결(1단계 정적 대조는 9/1 오전 완료)
- **CTA 분기 구현** — 코드 2줄(삼항), 문서 4개 파일 동기화: PRD-11(CPY-WF-016 신설) · SCR-004(EL-WF-007 조건 열·REQ-WF-004 AC-1 소유자 분기·전환 표·이탈 경로) · PRD-15(§UTM 표에 소유자 행 추가)
- 검증: `npm run lint` 0건 · `npm test` 31/31 pass · `npm run build` 통과
- **발견 경로가 기록할 값**: 이 결함은 코드 리뷰가 아니라 **GA4 DebugView 실검증 중에 눈으로 잡혔다**. Day 14 3축 리뷰·Day 15 정적 대조를 통과한 항목이고, 스펙이 침묵한 자리라 문서 대조로는 절대 안 걸린다. "실제로 한 번 써본다"가 정적 검사를 대체하지 못하듯, 정적 검사도 실사용을 대체하지 못한다
- ⚠️ **잔여 [본인]**: 롤백 리허설 1회 · CSV 백업 1회 (Day 16, 각 5분) · DebugView 테스트로 만든 카드 삭제(`/me` — **탈퇴는 금지**, cascade로 런칭 카드 5장 전멸)

## 2026-09-01 — Day 15 마감: 소유자 CTA 프로덕션 반영 + 롤백 리허설 1회

**무엇을**: ①`fix/owner-cta-copy` main 머지 → 프로덕션 배포 ②Vercel 롤백 리허설 1회(본인) ③리허설로 드러난 런북 구멍 보강

**어떻게**: 머지 후 프로덕션에서 비로그인 CTA(UTM 유지)·주요 5경로 200·Sentry 소스맵 업로드 3건을 실측으로 확인했다. 리허설은 `vercel rollback <직전배포>` → `stackd.kr` 눈 확인 → `vercel promote <최신배포>` 복귀 4단계로 돌렸고, 프로덕션이 최신 배포로 되돌아온 것까지 `vercel ls --prod`로 확인했다.

**왜**: 리허설을 **런칭 전에** 한 이유는 지금이 트래픽 0에 가까운 유일한 구간이기 때문이다. 런칭일에 처음 해보는 롤백은 리허설이 아니라 도박이다. 그리고 **되돌릴 대상(직전 배포)이 명확할 때** 해야 절차만 검증되고 변수가 안 섞인다.

**결과**:
- 프로덕션 `stackd-hyl3g42om…` Ready(35s) — 비로그인 CTA `utm_source=card` 서버 HTML에 존재(회귀 없음), `/` `/card` `/workflows` `/privacy` `/terms` 전부 200, E2E 프리뷰 대상 1 passed(3.4s)
- **Sentry 소스맵 업로드 3건 매칭** — 업로드 실패는 빌드를 깨지 않는 설계(자격증명 없는 환경 보호)라 매 프로덕션 배포마다 능동 확인이 필요한 항목이다
- **롤백 리허설 4단계 전부 정상, 1분 미만**
- **런북 결함 1건 보강**: `docs/runbook.md` §1에 **복귀 명령(`vercel promote`)이 없었다**. 되돌리는 법만 있고 돌아오는 법이 없으면 리허설이 그대로 사고가 된다 — 장애 중에 읽는 문서에서 이건 치명적 누락이다. **리허설을 안 돌렸으면 런칭일에 발견했을 것**이고, 그때는 프로덕션이 옛 버전에 묶인 채로 검색하고 있었을 것이다
- ⚠️ **잔여 [본인]**: CSV 백업 1회 · DebugView 테스트 카드 삭제(`/me` — 탈퇴 금지)

## 2026-09-01 — Day 15~16 블록 종료: CSV 백업 1회 + 테스트 카드 정리

**무엇을**: ①Supabase CSV 수동 백업 1회(본인 다운로드 → 런북 규칙 경로로 이관) ②백업 무결성 검증 ③DebugView 테스트로 생성된 카드 삭제(본인)

**어떻게**: 대시보드 Table Editor에서 받은 `workflows_rows.csv`·`feedback_rows.csv`를 `~/backups/stackd/2026-09-01/`(runbook §2 보관 규칙)로 옮기고, `python3 -c csv`로 파싱해 행 수·컬럼 수를 셌다. 삭제 대상은 백업 CSV의 `created_at`으로 특정했다 — 8/28 시드 5장이 초 단위까지 동일 타임스탬프로 묶여 있어 9/1 생성분 1장이 그대로 분리된다.

**왜**: **백업을 먼저 받고 삭제를 나중에** 한 순서가 핵심이다. 지금 CSV에는 삭제될 카드까지 들어 있어서, 잘못 지웠어도 원본이 남는다. 반대 순서였으면 삭제 사고에 대한 복구 지점이 없다. 그리고 행 수를 `wc -l`이 아니라 csv 파서로 센 이유는 `situation`·`steps.detail`이 `white-space: pre-wrap` 다행 텍스트라 **셀 안에 개행이 들어 있기 때문**이다 — 줄 수와 레코드 수가 다르고, 여기서 잘못 세면 "다 받았다"는 착각을 한다.

**결과**:
- 백업: `workflows_rows.csv` **6행/16컬럼**(런칭 5장 + 테스트 1장) · `feedback_rows.csv` **3행/7컬럼**. repo 유입 0건 확인(`git ls-files '*.csv'` 비어 있음 — 핸들·피드백 본문 포함이라 커밋 금지 대상)
- 삭제 검증(프로덕션 실측): `/card-detail/29cb9972` **404** · `/workflows`에 "테스트3" **0건** · sitemap `card-detail` **4건**(공개 런칭 카드 그대로, 비공개 1장은 원래 미포함)
- **PLAN "장애 대비 3종" 전부 종료** — 문서화(runbook) · 롤백 리허설 1회 · DB 백업 1회
- **Day 15~16 블록 종료** — sentry+알림 / 이벤트 검수(정적+DebugView) / E2E 1개 / 피드백 채널 / 법적 고지 / OG / 장애 대비 3종 전 항목 완료. 다음은 Day 17(9/3) 출시 전 기술 점검

## 2026-09-01 — Day 15 EOD: Notion 5건·Obsidian 5건 등록 + Day 13 이월 종결

**무엇을**: ①Day 15 EOD 정리(Notion 과정요약 1 + 트러블슈팅 4) ②Obsidian TIL 신규 4 + 기존 1건 보강 ③Day 13 GSAP 총정리 이월분 종결

**어떻게**: `/eod 15`로 당일 컨텍스트(커밋 15개)를 수집해 원고 `_notion-원고-stackd-Day15.md`(156줄)를 먼저 쓰고 **승인 후에만** Notion에 등록했다(외부 쓰기 게이트). TIL은 쓰기 전에 기존 129개를 키워드로 grep해 중복을 판정했다 — `롤백`·`promote`·`런북`·`UTM`·`어트리뷰션`·`DebugView`는 매칭 0건이라 신규, "스펙 공백"은 8/31 노트에 이미 있어 **새 파일 대신 `## 추가 사례` 보강**으로 처리하고 두 노트를 `[[ ]]`로 양방향 연결했다.

**왜**: EOD의 "Day"는 **달력 하루**고 PLAN의 "Day 15~16"은 **작업 묶음**이라 단위가 다르다. 블록이 하루 일찍 닫혔다는 사실은 Day 16 EOD를 미리 쓰는 게 아니라 Day 15 과정요약의 "남은 것" 칸에 한 줄로 남긴다. Day 13 총정리를 새로 쓰지 않은 이유도 같은 규칙 계열이다 — 이미 TIL 3건으로 존재하는 지식의 요약본은 링크 그래프를 갈라놓기만 한다.

**결과**:
- **Notion 5건** — 과정요약 1(태그 배포) + 트러블슈팅 4(TS-D15-1~4). 태그는 고정 4종(gsap/성능/기획/배포) 밖으로 나가지 않았고 유형·상태도 2종 고정 준수
- **Obsidian 신규 4** — 롤백 절차서 왕복 / 내부 링크 UTM과 소유자 클릭 / 모션 설계의 기각 목록(Day 13 이월) / 다행 텍스트 CSV 행 수. **보강 1** — 스펙 공백 TIL에 "스펙 공백은 위반으로도 보고되지 않는다" 사례 추가. 129 → 133개
- **도구 응답 압축에 속을 뻔한 건 1건**: 생성 직후 반환된 제목이 잘려 보여(`TS-D15-4 아니었다`) 결함으로 의심했는데, 같은 행을 SQL로 재조회하니 **또 다르게** 잘려 보였다. 두 읽기 경로가 서로 불일치하면 원인은 데이터가 아니라 읽기 경로다 → 단건 `fetch`로 확인한 결과 제목은 `—` 포함 온전. **손실적 읽기 경로의 출력으로 결함을 판정하면 없는 결함을 만든다**
- **Day 13 이월 종결** — `TODO.md:127` `[~]` → `[x]`

## 2026-09-02 — 기본 OG 태그라인 한국어화 + 위생 잔여 2건 종결

**무엇을**: ①`DefaultOg`의 영문 태그라인을 한국어로 교체 ②그 문장 전용 서브셋 폰트를 번들에 내장 ③`TODO.md` 위생 블록 잔여 2건(favicon·OG 한글) 종결 ④`PLAN.md` Day 16 체크리스트 종결

**어떻게**: Google Fonts `css2?...&text=`로 `내 AI 워크플로우를 카드 한 장으로 — stackd.kr`에 실제 쓰인 글자만 서브셋해(woff 28.9KB) `lib/og-font.ts`에 base64로 박았다 — `lib/og-wordmark.ts`가 이미 쓰는 패턴이다. 폰트 옵션은 `DEFAULT_OG_FONTS`로 `lib/og.tsx`에서 함께 내보내 호출부가 각자 조립하지 않게 했다. 요청 UA는 `app/api/og/route.tsx:22`의 `LEGACY_UA`를 그대로 썼다(woff2를 받으면 Satori가 못 읽는다).

**왜**: `DefaultOg`는 **정적 기본 OG와 `/api/og`의 ERR-OG-001 폴백 두 곳**이 공유한다. 컴포넌트에 한글을 넣으면서 폰트를 한쪽에만 넘기면 다른 쪽이 통째로 빈칸이 되는데, 폴백은 에러가 났을 때만 뜨니 배포 후에도 한참 안 들킨다. 그래서 폰트를 인자로 흘리지 않고 `DEFAULT_OG_FONTS` 한 상수로 묶어 "빠뜨릴 수 없는" 형태로 만들었다. 런타임 fetch를 안 쓴 이유는 둘 다 안 되기 때문이다 — 정적 OG는 빌드 타임이라 fetch가 없고, 폴백은 **폰트 fetch가 실패해서 도달한 경로**라 거기서 또 네트워크를 타면 같은 이유로 죽는다.

**결과**:
- 실측 2건 모두 1200×630 PNG 한글 정상 렌더: 빌드 산출 `.next/server/app/opengraph-image.body` · `/api/og?id=zzz`(BR-023 정규식 불통과 → 폴백 강제 발화) 200
- `npm run lint` 0 / `npx tsc --noEmit` 0 / `npm test` **31 pass**
- **favicon 잔여는 실제로는 8/28에 끝나 있었다**(`22d4886`) — `TODO.md`의 8/26자 잔여 메모를 해소 커밋에서 안 지워 9/2까지 미결로 보였다. 잔여 표시는 해소한 커밋과 같은 커밋에서 지운다
- 이로써 8/26 위생·SEO 블록 `[~]` → `[x]`, **Day 17로 이월 예정이던 2건이 사전 소멸**
- ⚠️ 잔여: 문구를 바꾸면 `lib/og-font.ts` 서브셋을 다시 받아야 한다(없는 글자는 빈칸). 두 파일 주석에 명시

## 2026-09-02 — Day 16 EOD: Notion 3건·Obsidian 3건 등록

**무엇을**: ①Day 16 EOD 정리(과정요약 1 + 트러블슈팅 2) ②Obsidian TIL 신규 1 + 기존 2건 보강

**어떻게**: `/eod 16`으로 당일 커밋 5개를 수집해 원고 `_notion-원고-stackd-Day16.md`를 먼저 쓰고 **승인 후에만** Notion에 등록했다(외부 쓰기 게이트). TIL은 쓰기 전에 기존 118개를 `OG`·`satori`·`폰트`·`서브셋`·`폴백`·`두부`·`잔여`·`미결`로 grep해 중복을 판정했다 — `OG 폰트 서브셋`과 `유령 잔여 항목`은 매칭이 있어 **신규 대신 `## 추가 사례` 보강**, `폴백의 자원 의존`만 매칭 0건이라 신규로 만들었다.

**왜**: 보강한 두 건은 **기존 노트의 결론이 오늘 사례에서 성립하지 않는** 자리였다. `TIL-OG-폰트는-런타임-서브셋...` 33행은 "정적 OG는 readFile, 용량 무관"이라고 단정했는데, 그 컴포넌트를 폴백과 공유하는 순간 readFile도 fetch도 못 쓴다. `TIL-SSOT-선언이-없으면...`은 증상(유령 잔여)이 같지만 원인 계열이 달라서(문서 갈라짐 → 커밋 위생) **왜 기존 처방으로 안 막히는지**를 같이 적었다 — 안 적으면 다음에 잘못된 처방을 꺼낸다.
...
## 2026-09-04 — 런칭 채널 3곳 확정 (GeekNews·디스콰이엇·Threads)

**무엇을**: Day 18~19 런칭 채널을 게시판 2곳 + 소셜 1곳으로 확정하고, UTM 규격·계획서·Day 1 결정 기록 3개 문서를 동기화

**어떻게**: PLAN.md는 "채널 1~2개 선정(국내: 디스콰이엇 + GeekNews)"이었고 Threads는 어느 문서에도 없었다. PRD-15 §UTM의 런칭 포스트 행을 **게시판/소셜 2행으로 분리** — 게시판은 `utm_source=disquiet`/`geeknews` + `utm_medium=community`, Threads는 `utm_source=threads` + `utm_medium=social`. PLAN.md는 Day 18~19 작업 행·일정 매핑 행·진행 체크리스트 3곳, day1-decisions.md는 §유입 전략에 날짜 붙인 갱신 줄을 추가(기존 "2026-08-17 재정의" 패턴 답습)

**왜**: medium을 분리한 이유는 Threads가 게시판이 아니라 소셜 피드라서다 — 성격이 다른 채널을 같은 medium으로 묶으면 판정일(9/22)에 "커뮤니티 게시가 먹혔나"와 "소셜 확산이 먹혔나"를 구분할 수 없다. 채널을 3곳으로 늘리면 킬 크라이테리아 ③(표본 300명 미달 시 채널 1회 추가 게시)의 예비가 사라지는데, **Threads는 재게시 비용이 0이라 예비 역할을 겸한다** — 게시판 2곳은 1회씩만 태우고 아껴두는 원래 설계가 유지된다

**결과**:
- 문서 3개 갱신: `docs/PLAN.md`(3곳) · `docs/prd/15_분석및추적.md`(UTM 표 1행 → 2행, version 2.0.0 → 2.1.0) · `docs/day1-decisions.md`(§유입 전략)
- 코드 변경 0 — UTM은 GA4가 자동 수집하는 쿼리 파라미터라 계측 코드에 손댈 곳이 없다
- 유입 전략 3층 구조는 불변: 주채널 = 카드 확산 루프(`utm_source=card`) / 초기 부스트 = 게시판 2곳 / 예비 = Threads 재게시

## 2026-09-04 — 런칭 포스트 초안 3종 (GeekNews·디스콰이엇·Threads)

**무엇을**: Day 18~19의 "런칭 포스트 작성" 중 Claude 몫(초안·링크표·소재 고정)을 `docs/launch-posts.md`로 산출. 본인 몫(제목 확정·목소리 리라이팅·OG 실측)은 문서 안에 체크박스로 남김

**어떻게**: 세 글을 **같은 사실의 다른 축**으로 나눠 썼다 — GeekNews = 구현 판단(Safari `transform: scale()`이 레이아웃 박스를 안 줄인 건, 카드 전환 지연의 원인이 Vercel 함수 리전 iad1이었던 건, Next `openGraph`가 병합이 아니라 교체인 건), 디스콰이엇 = 과정(아이디어 3번 뒤집기·Day 3 스코프 재판정·런칭일 불변), Threads = 이미지 + 3~5줄 훅. 문서 상단에 **소재 표**를 두고 포스트의 모든 숫자를 거기서만 가져오게 고정했다(커밋 220·카탈로그 268·LCP 824ms·Lighthouse 100·Day 14 치명 5건·Day 17 Safari 3건 — 전부 실측·리포 조회로 확인). 링크표는 PRD-15 §UTM을 그대로 옮겨 채널 3 × (홈·예시 카드) 6종

**왜**: 세 채널에 같은 글을 복붙하면 `utm_medium`을 나눠도 해석이 안 된다 — 성과 차이가 채널 특성 때문인지 글이 그 채널과 안 맞아서인지 구분 불가라, 채널 분리 계측 자체가 무의미해진다. 숫자를 소재 표로 못 박은 이유는 리라이팅 중 "약 200커밋" 같은 반올림이 들어가면 세 글 사이에서 숫자가 어긋나기 때문. Threads 3번 글을 첫 게시에서 빼둔 것은 킬 크라이테리아 ③ 재게시 소재를 남기기 위함이다

**결과**:
- `docs/launch-posts.md` 134줄 — 링크표 · 소재(실측 근거) · 포스트 3종 · 게시 전 체크 5항목
- **게시용 링크 12개 전건 UTM 부착 확인** (`grep`으로 UTM 없는 링크 0건 — 검출된 2건은 규칙 문장과 정규식 자체로 오탐)
- ~~`{id}` 플레이스홀더는 Day 19에 치환~~ → **당일 해소**: 0번 카드가 프로덕션에 아예 없다는 사실이 드러나 `lib/hero-card.ts` 데이터를 초안 복원(BR-019) 경로로 주입해 저장(`934c9228`), 8곳 전건 치환. 플레이스홀더가 '값을 모른다'로 읽혀 '대상이 없다'를 가렸다
- ⚠️ 잔여(본인): 제목 후보 확정 · 목소리 리라이팅 · 채널 3곳 작성창에서 OG 미리보기 실측(Threads 포함)
...
**결과**:
- 테스트 39개 통과 · lint 0 · tsc 0 · build 통과
- 서버가 "카탈로그 등재 도구는 custom 불가"를 강제하지 못하는 한계를 BR-026에 명시 — 안 적으면 나중에 모순 데이터를 버그로 오진한다
- 기존 카드 6장(`custom` 없음) 정상 렌더 확인 — 하위호환 실증
- ⚠️ 런칭 D-4 작업. 머지 판정은 계획서 §머지 판정 기준을 따랐다 — **머지 완료(2026-09-06)**. 프리뷰 배포에서 모바일·데스크톱·서버리스 OG 시각 확인 통과. 실제 Safari(WebKit) 캡처는 macOS 자동화·화면 기록 권한 부재로 실패해 사용자 육안 확인으로 대체(코드는 표준 flex/truncate만 사용, Day 17 회귀 원인이었던 tan(atan2())·컨테이너 쿼리는 미접촉)

## 2026-09-05 — 버퍼일 BR-026 EOD 기록 (Notion 5페이지 / Obsidian TIL 3신규·1보강, 2026-09-09 소급 작성)

### 작업 내용

- **무엇을**: 9/4 저녁~9/5 버퍼일의 커스텀 도구 토글(BR-026) 9커밋 EOD. **EOD 커버리지 감사에서 마지막까지 남아 있던 미기록 구간**이다
- **어떻게**:
  - **번호 경계를 명시했다** — PLAN상 버퍼일이라 Day 번호가 없다. 작업이 **Day 18(9/6)에 main 머지·문서 반영으로 종결**됐으므로(`9e6a20d`) 트러블슈팅 번호를 `TS-D18-4~7`로 이어 붙이고, **날짜 속성에는 실제 발생일(9/4·9/5)**을 넣어 사실을 보존했다. 과정요약 제목은 Day 번호 없는 기존 선례(「버퍼 소진 2」·「Day 5 잔여 종결」)를 따랐다
  - **Notion 5페이지** — 과정요약 1 + 트러블슈팅 4. 태그는 캐노니컬 4종 내 `기획` 3건, TS-D18-4·5는 미태깅
  - **Obsidian TIL 신규 3** — 계획서에 든 코드는 검토 대상이 아니라 실행 대상이다 / UI 노출 조건은 저장하지 말고 파생 계산으로 매번 판정한다 / 강제되지 않는 규칙은 강제 주체를 함께 적어야 한다
  - **Obsidian 보강 1** — `TIL-Node-26-네이티브-TS면-테스트-러너-의존성이-0이-된다`에 「러너가 0이 된 대가: 번들러가 주던 편의도 0이다」 추가. 경로 별칭 미해석과 JSON import 속성 요구는 그 노트의 결론이 안 다루던 자리다
- **왜**: 이 블록의 값어치는 구현이 아니라 **착수 전에 잡은 것들**에 있다. 계획 단계에 실제 코드를 읽어 설계 결함 2건, 프로브로 계획서 코드 결함 2건, 문서 대조로 표 결함 1건을 잡았다. 전부 구현자에게 넘기기 전에 처리돼 **재작업 라운드가 0**이었다

### 작업 결과

- **Notion 5페이지 등록 완료**, 5건 전부 단건 조회로 제목 확인
- ⚠️ **이스케이프 오타 재발 1건** — TS-D18-4 제목·본문의 `켤`이 `콜`로, `끌`이 `딠`로 들어갔다. 9/9 Day 18에서 같은 사고(`컬럼`→`컴럼`)를 겪고 "한글을 그대로 넣는다"로 회피했는데, **Day 19·20에서 두 번 성공한 뒤 이 배치에서 조용히 옛 방식으로 돌아갔다.** 제목·본문 전건 치환 후 검색으로 잔여 0 확인(대조군 `끌 수도` 검색이 해당 문단을 반환하는 것까지 확인)
- **Obsidian 180 → 184 파일**(TIL 신규 3 + 원고 1), 새로 건 위키링크 전수 실재 확인 — **깨진 링크 0**
- 원고 스테이징: `_notion-원고-stackd-BR026.md`

### 부수 확인

- **이로써 Notion 기준 미등록 구간이 0이 됐다.** Day 0~20 + 버퍼일 블록 전부 등록 완료. 남은 것은 진행 중인 Day 21(9/9)뿐이다
- Day 1(8/12) Obsidian 커버리지는 **오탐이었다** — 노트 6건이 프로젝트명을 본문에 쓰지 않아 자동 분류에서 빠졌을 뿐, 도메인 선점률·Route53 kr 제약 등 그날 작업 그대로다. 볼트를 두 프로젝트가 공유해서 생기는 판정 오류다

---

## 2026-09-06 — 라이브러리 카드 겹침 수정 + gap 축소·카드 확대 + 페이지네이션을 shadcn Pagination으로 교체

**무엇을**: ①`/workflows` xl 이상에서 카드가 옆 카드와 겹치는 버그 수정 ②`gap-8→gap-4` 축소 + 카드 크기 확대 ③"더 보기" 링크 방식을 shadcn `Pagination`(번호 이동)으로 교체하는 과정에서 발견한 PostgREST 416 버그 수정

**어떻게**:
- **겹침 버그**: `--s`(카드 축소 배율)가 `lg:0.52`(3열에 맞게 튜닝된 값)에서 `xl:0.6`으로 다시 커졌는데, `/workflows`는 `lg:grid-cols-3`에서 컬럼 수가 고정돼 `xl`에서도 안 바뀐다. 실측(1309px 뷰포트): 셀 폭 298.67px에 카드 335.99px — 37.3px 초과해 gap을 뚫고 옆 카드를 침범. `xl:[--s:0.6]` 삭제로 `lg` 값이 이어지게 해결(291.19px, 셀 안).
- **gap 축소·카드 확대**: 컨테이너가 1024px 고정(DESIGN.md 단일 컨테이너)이라 gap을 줄인 만큼만 카드를 키울 수 있다. `chrome-devtools` MCP로 640·768·1024·1440px 4개 뷰포트를 실제 리사이즈해 컬럼 폭을 재고, 여유 5~10px만 남기고 `--s` 재계산(`sm 0.49→0.5, md 0.6→0.61, lg 0.52→0.54`).
- **페이지네이션 교체**: `npx shadcn add pagination`으로 컴포넌트 추가(생성된 `import { cn } from "cn"`은 오류라 `@/lib/utils`로 수정, `button.tsx` 덮어쓰기는 거부). `lib/paginate.ts`에 TDD로 `pageCount`·`pageWindow`(처음/끝/현재±1 + ellipsis) 순수 함수 추가, 13행 프로브(`splitPage`) 대신 정확 개수가 필요해졌다. **버그 발견**: `count: "exact"`를 `range()`와 같은 쿼리에 같이 쓰면 `from`이 총 개수를 넘을 때 PostgREST가 200 대신 **416**(`PGRST103`)을 던진다 — curl로 재현(`from=7·total=6`). 하필 이게 **지금 프로덕션 상태(6건, PAGE_SIZE 12)에서 `?page=2`만 쳐도 걸리는** 경로였다. count 쿼리를 `head: true`로 분리해 먼저 유효 페이지인지 판정한 뒤에만 본문 쿼리를 실행하도록 재구성.

**왜**: gap-8→gap-4는 사용자 요청 그대로지만, 순서를 지켰다 — 방금 고친 xl 겹침 버그와 같은 계산을 반대 방향으로 쓰는 변경이라 컬럼 폭을 먼저 실측하지 않았다면 그 버그를 그대로 재현했을 것. 페이지네이션 쿼리 분리는 "매 요청 1쿼리"라는 기존 설계 목표보다 정확성이 우선이다 — 416을 그냥 `error`로 처리하면 `!error` 가드에 걸려 범위 초과 리다이렉트가 조용히 안 되고 에러 화면이 뜬다.

**결과**:
- 테스트 40개 통과(`pageCount`·`pageWindow` 신규 4개 포함) · lint 0 · tsc 0
...
## 2026-09-06 — Day 18 EOD 기록 (Notion 4페이지 / Obsidian TIL 2신규·1보강, 2026-09-09 소급 작성)

### 작업 내용

- **무엇을**: Day 18(9/6) EOD 기록 블록 — 커밋 15개(`9e6a20d`~`47265b7`)를 소스로 트러블슈팅 3건·과정요약 1건을 이중 기록. **런칭 이후 소급 작성**이며 승인 게이트는 사용자 지시로 생략
- **어떻게**:
  - 등록 전 Notion을 날짜 범위로 조회해 9/4 이후가 **전부 미등록**임을 확인하고 착수했다(Day 10~14에서 위양성을 네 번 겪은 뒤 굳은 절차)
  - **위 9/6 블록이 오후·저녁분만 덮고 있었다** — 심야 리팩터 9커밋(23:47~23:48)은 문서에 없어 커밋 메시지와 diff에서 복원했다. 10파일 순삭제 34줄(추가 54 / 삭제 88)
  - **Notion 4페이지** — 과정요약 1 + 트러블슈팅 3(TS-D18-1~3). 태그는 캐노니컬 4종 중 해당 없음으로 판단해 **전건 미태깅**(Day 6 판례 — 추측 대신 미태깅)
  - **Obsidian TIL 신규 2** — PostgREST가 count exact와 range를 같이 쓰면 범위 초과를 416으로 던진다 / 미래 대비 조건문은 그 미래가 오기 전까지 테스트되지 않는다
  - **Obsidian 기존 1건 보강** — `TIL-transform-scale은-페인트만-줄이고-레이아웃-폭은-그대로다`에 「배율과 컬럼 수가 다른 브레이크포인트에 걸리면 xl에서 되살아난다」 추가. 이 노트는 8/28에도 같은 화면의 xl 초과를 다뤘는데, 9/6 건은 **계산 실수가 아니라 선언이 걸린 브레이크포인트 불일치**라 원인이 다르다
- **왜**: 416 건은 **당시 프로덕션에서 이미 재현되던 결함**이다. 공개 카드 6건에 `PAGE_SIZE` 12라 `?page=2`만 쳐도 걸렸고, 카드가 13건을 넘으면 영영 안 보인다 — 런칭 초기의 적은 데이터가 발현 조건이었고 그때가 사람들이 처음 들어오는 시점이다. 이런 건 재현 조건까지 남겨야 재사용 가치가 생긴다

### 작업 결과

- **Notion 4페이지 등록 완료** — 유형·상태 2종 고정 준수, 전건 상태 `완료`
- ⚠️ **오타 1건 발생·수정**: 도구 인자에 한글을 `\uXXXX`로 손수 이스케이프하다 `컬럼`을 **`컴럼`**으로 잘못 적어 TS-D18-1의 제목과 본문, 과정요약 본문에 들어갔다. `instr()`로 제목 74번째 위치를 특정하고 전건 치환한 뒤, 검색으로 잔여 0을 확인(대조군으로 `컬럼` 검색이 두 페이지를 반환하는 것까지 확인). **같은 문장을 파일 쓰기 도구로 작성한 원고에는 오타가 없었다** — 이스케이프가 유일한 발생 지점
- **Obsidian 165 → 168 파일**(TIL 신규 2 + 원고 1), 새로 건 위키링크 9개 전수 실재 확인 — **깨진 링크 0**
- 원고 스테이징: `_notion-원고-stackd-Day18.md`
- ⚠️ **Notion SQL 조회 쿼터 소진** — 검증 후반부는 단건 `fetch`와 검색으로 대체했다. 다음 EOD 세션에서 조회가 막히면 같은 우회를 쓴다

### 부수 확인

- Day 18 심야 리팩터가 `docs/history.md`에 없던 것은 **기록 공백**이다. 이 EOD가 그 구간의 유일한 기록이므로, 나중에 그 커밋들을 추적할 때 Notion 과정요약을 봐야 한다
- 9/7(Day 19)은 커밋 1개(`eb91ef9`, UTM 무결성 검증 표시)뿐이라 단독 EOD 물량이 아니다. Day 19 항목은 Day 18 과정요약의 "남은 것"에 흡수했다

---

## 2026-09-07 — Day 19 EOD 기록 (Notion 4페이지 / Obsidian TIL 3신규, 2026-09-09 소급 작성)

### 작업 내용

- **무엇을**: Day 19 EOD — 트러블슈팅 3건·과정요약 1건을 이중 기록. **런칭 이후 소급 작성**이며 승인 게이트는 사용자 지시로 생략
- **어떻게**:
  - **Day 경계를 달력이 아니라 작업 단위로 잡았다.** 달력상 9/7은 커밋 1개(`eb91ef9`)뿐이지만, PLAN이 Day 19에 배정한 산출물(런칭 포스트·채널 준비)은 **버퍼일 9/4에 선행 완료**되고 9/7에 검증·판정으로 닫혔다. 그래서 9/4 런칭 준비 3커밋 + 9/7 검증 1커밋을 한 단위로 덮었다
  - **Notion 4페이지** — 과정요약 1 + 트러블슈팅 3(TS-D19-1~3). 태그는 캐노니컬 4종 내 `기획` 3건, TS-D19-2만 미태깅
  - **Obsidian TIL 신규 3** — 외부 플랫폼 게시 계획은 콘텐츠보다 접근 권한으로 먼저 거른다 / 플레이스홀더는 값 미정을 표시하지 대상 존재를 보증하지 않는다 / 인용값에는 측정 시점을 함께 적는다
- **왜**: Day 19의 핵심 사건은 코드가 아니라 **판정**이다. 게시 이틀 전에 채널 3곳 중 2곳이 플랫폼 조건(GeekNews 가입 후 1주 대기·디스콰이엇 제품 승인)으로 탈락해 Threads 단독으로 축소됐다. 이건 일정 문제가 아니라 **채널 표에 "게시 가능 시점" 열이 없었던** 문서 구조 문제라, 원인을 그 층위로 적어야 다음 프로젝트에서 재발을 막는다

### 작업 결과

- **Notion 4페이지 등록 완료** — 4건 전부 단건 조회로 제목 온전성 확인. **오타 0** — Day 18에서 유니코드를 손으로 이스케이프하다 낸 사고를 반영해 이번엔 도구 인자에 한글을 그대로 넣었다
- **Obsidian 172 → 176 파일**(TIL 신규 3 + 원고 1), 새로 건 위키링크 8개 전수 실재 확인 — **깨진 링크 0**
- 원고 스테이징: `_notion-원고-stackd-Day19.md`
- **TS 3건**: ①채널 2곳 탈락(리드타임 미확인) ②포스트가 링크한 예시 카드가 프로덕션에 부재(플레이스홀더가 대상 부재를 가림) ③소재 표 수치가 게시 전에 낡음(SSOT는 신선도를 보장하지 않는다)

### 부수 확인

- ⚠️ **이 EOD가 덮지 않는 블록**: 9/4 저녁~9/5의 커스텀 도구 토글(BR-026) 9커밋. 런칭 준비가 아니라 기능 추가라 성격이 달라 분리했다. `history.md` 9/4 블록에 작업 기록은 있으나 **EOD 등록은 없다**
- Day 18~19 EOD로 9/6·9/7이 닫혔다. Notion 기준 남은 미등록 구간은 **9/4~9/5(BR-026)와 9/8(Day 20)** 둘이다
...
- **브랜드 검색 대비(같은 날)**: "stackd" 영문은 글로벌 경합어(자판기 앱·버거집·핀테크…)라 단기 1위 비현실적 → 목표를 **"스택드"·"stackd.kr"**로. 사이트에 한글 브랜드명이 0곳이라 title 3곳(layout·홈·OG)을 "Stackd(스택드) — …"로, `WebSite.alternateName: "스택드"` + **`Organization`**(logo=`apple-icon.png` 180×180 PNG, sameAs=GitHub 레포) 추가. PRD-04·day3 스펙의 같은 문구 동기화. 사이트링크는 Google 자동 생성이라 지정 불가. 남은 사용자 몫: 네이버 서치어드바이저 등록(별도 엔진) · 9/10 포스트에 "Stackd(스택드)" 표기 통일
- **네이버 서치어드바이저 등록 완료(같은 날, 본인)**: 소유확인 화면에 DNS 옵션이 없어(HTML 파일/태그 2종) **HTML 태그**로 — `layout.tsx` `metadata.verification.other`에 한 줄(`962d24b`), 11커밋 main 머지·프로덕션 배포 후 실측 → 소유확인 통과 · `sitemap.xml` 제출 · 웹 페이지 수집 3건. 남은 사용자 몫: 9/10 포스트 "Stackd(스택드)" 표기 통일 · Threads URL(sameAs)

## 2026-09-08 — Day 20 EOD 기록 (Notion 6페이지 / Obsidian TIL 3신규·2보강, 2026-09-09 소급 작성)

### 작업 내용

- **무엇을**: Day 20(9/8) EOD — 25커밋을 트러블슈팅 5건·과정요약 1건으로 이중 기록. 남아 있던 미기록 Day 중 가장 큰 덩어리다
- **어떻게**:
  - 커밋을 **클러스터 7개**로 갈랐다 — 런칭 준비 마감 → 문서 위생 → 보안 점검·모의 침투 → 런칭일 통일 → SEO 1차 → SEO 2차(100점 보완) → 브랜드 SEO·네이버
  - **Notion 6페이지** — 과정요약 1 + 트러블슈팅 5(TS-D20-1~5). 태그는 캐노니컬 4종 내: 과정요약 `배포`+`기획`, TS-D20-1 `배포`, TS-D20-3 `기획`, 나머지 2건 미태깅
  - **Obsidian TIL 신규 3** — 자기 인프라를 침투 테스트하면 자기 방어 장치가 먼저 반응한다 / 크롤 기반 감사는 외부 시스템의 등록 상태를 못 보고 부재로 보고한다 / 오래된 브랜치는 머지 대상이 아니라 메모 대상이다
  - **Obsidian 보강 2** — `TIL-SSOT-선언이-없으면-같은-사실을-든-두-문서는-갈라진다`에 favicon 유령의 **3차 재발과 규칙 승격** 추가 / `TIL-권고는-도구가-실제로-받는-옵션을-확인한-뒤에만-절차에-적는다`에 네이버 소유확인 DNS 옵션 부재 사례 추가
- **왜**: Day 20의 사건 5건은 성격이 전부 다르다 — 인프라(WAF), 도구 판정(오탐), 문서 위생(3차 재발), git(스테일 브랜치), 외부 플랫폼(UI 불일치). 하나로 묶으면 각각의 재현 조건이 사라져 재사용이 안 된다

### 작업 결과

- **Notion 6페이지 등록 완료** — 6건 전부 단건 조회로 제목 온전성 확인, **오타 0**. Day 18 사고 이후 도구 인자에 한글을 그대로 넣는 방식이 두 번 연속 유효했다
- **Obsidian 176 → 180 파일**(TIL 신규 3 + 원고 1), 새로 건 위키링크 전수 실재 확인 — **깨진 링크 0**
- 원고 스테이징: `_notion-원고-stackd-Day20.md`
- **TS 5건**: ①모의 침투가 자기 WAF에 막힘(방어 발동이 곧 결과) ②SEO 감사의 GSC 미등록 High는 오탐(크롤은 외부 상태를 못 본다) ③favicon 유령 3차 재발 → 상시 규칙 신설 ④옛 main 브랜치를 머지 대신 의도만 재적용 ⑤네이버 소유확인에 조사한 DNS 옵션 부재

### 부수 확인

- Day 20 과정요약의 "남은 것"에 **CSP 도입**을 명시했다. 런칭 당일 GSAP·GA4·Supabase 파손 위험으로 의도적으로 미룬 항목이라, 기록하지 않으면 영구 미도입이 된다
- ⚠️ **여전히 미등록인 구간**: 9/4~9/5 커스텀 도구 토글(BR-026) 9커밋. 런칭 준비가 아니라 기능 추가라 Day 19에서 분리했고 아직 EOD가 없다
- Notion SQL 조회 쿼터는 여전히 소진 상태 — 검증은 단건 `fetch`로 대체했다

---

## 2026-09-09 — Day 21 계획 변경: 초기 유저 인터뷰 섭외 → 본인 직접 테스트로 대체

**무엇을**: PLAN.md Day 21의 "초기 유저 3~5명 인터뷰 섭외"와 런칭 후 2주 루틴의 "인터뷰 실행"을 **본인 직접 테스트**로 대체 (2026-09-09 사용자 판정). 섭외·인터뷰 항목은 소멸.

**어떻게**:
- 수정 4곳: Week 3 표 Day 21 행(203) · 런칭 후 운영 루틴 1주차 항목(211) · 진행 체크리스트 Day 21(330) · 런칭 후 2주 루틴 체크(332). 전부 취소선 + 판정일·사유 병기로 원안 보존(잔여 표시 규칙과 동일하게, 지운 흔적이 아니라 판정 기록으로 남긴다)
- 대체 내용: 런칭 후 본인이 실사용 경로(만들기 → 저장 → 공유 → 재방문)를 순회하며 마찰 지점을 기록 → backlog.md 우선순위에 반영
- **대체되지 않는 것을 명시**: "처음 보는 사람이 어디서 멈추는가"(사용성 검증)는 본인 테스트로 확인 불가 — Day 14(2026-08-28 게릴라 → 본인 신규 계정) 판정과 동일한 한계. 자발적 피드백(댓글·문의 폼·Threads 반응)이 유일한 외부 신호가 되므로 **전건 기록**을 루틴에 추가
- 킬 크라이테리아·판정일(9/24)은 GA4 실데이터 기준이라 **영향 없음** — 인터뷰는 backlog 우선순위 입력이었지 판정 입력이 아니었다

**왜**: 런칭 직후 인터뷰 대상 확보는 섭외 자체가 병목이고, 표본이 안 잡히면 준비 시간만 쓰고 산출물이 0이 된다. 본인 테스트는 확실히 실행되는 대신 사용성 축을 잃는데, 그 손실을 문서에 남겨야 나중에 "왜 사용성 데이터가 없나"를 복구할 수 있다.

**결과**:
- PLAN.md 4곳 갱신, 남은 `인터뷰` 언급은 Day 1 원문의 비유 표현 1건뿐(무관)
- 산출물 2종 신규: `docs/day21-self-test.md`(순회 시나리오 A~E 20항목 + 마찰 기록 템플릿 + 심각도 3단계) · `docs/kill-criteria-verdict.md`(판정 기준 3종 + 보조 지표 5종 + 판정 결과 4택, 9/24에 숫자만 채우는 형식)
- **판정 날짜 표기 정정**: "(2026-09-10 사용자 판정)"은 판정일보다 미래여서 불가능한 표기 — 실제 판정은 2026-09-08(history 같은 날 항목 ②). PLAN.md 2곳·launch-posts.md 2곳(**한 줄에 2회 등장**) 총 4곳 수정, 전 문서 잔존 0 확인
- 브랜치 `docs/plan-day21-self-test`

## 2026-09-13 — 개인정보처리방침 법정 기재사항 보완 (보호책임자·권익침해 구제 방법)

**무엇을**: `app/privacy/page.tsx`에 **6. 개인정보 보호책임자**와 **7. 권익침해 구제 방법** 2개 장을 신설하고 시행일을 갱신. 푸터(`components/site-footer.tsx`)는 **의도적으로 변경하지 않음**.

**어떻게**:
- 발단은 "타사 푸터처럼 사업자 정보를 더 넣어야 하나"라는 질문. 적용 법령을 항목별로 대조한 결과 **전자상거래법 제10조**(상호·대표자·주소·전화번호·사업자등록번호·통신판매업 신고번호)는 **통신판매업자** 대상이라 판매·결제가 없는 현 서비스에 비적용. 유료직업소개사업등록번호는 직업안정법 소관으로 무관. 즉 푸터에 추가할 법정 항목은 0건
- 대신 **개인정보보호법 제30조 1항**이 사업자 여부가 아니라 **개인정보처리자** 지위에 걸린다는 점에서 현행 처리방침을 재점검 → 5호(개인정보 보호책임자의 성명 또는 담당부서명 및 연락처)·7호(권익침해 구제 방법) 2건 누락 확인
- 6장: 조문이 "성명 **또는** 담당부서명"을 선택지로 두므로 실명 노출 대신 `담당 — 운영자`로 갈음. 연락처는 이미 전 페이지에 노출된 **문의·피드백**으로 연결
- 7장: 개인정보침해신고센터(118)·분쟁조정위원회(1833-6972)·대검 사이버수사과(1301)·경찰청 사이버수사국(182) 4기관
- 시행일 `2026-09-08 → 2026-09-13` 갱신 + 최초 제정일 이력 보존. 본문이 "개정되면 이 페이지의 시행일을 갱신해 알립니다"라고 스스로 약속하고 있어, 갱신하지 않으면 그 문장이 즉시 거짓이 된다

**왜**: 법정 기재사항은 제30조 1항에 1~7호로 **묶여** 있어서, 하나가 비어 있으면 이웃 호도 비어 있을 확률이 높다. 5호만 채우고 7호를 두면 같은 파일을 다시 열게 되므로 한 번에 처리했다. 푸터를 건드리지 않은 이유를 커밋 본문에 남긴 것은, **부재는 코드에 흔적이 없어** 반년 뒤 "왜 사업자 정보가 없나"를 물었을 때 `git log`가 유일한 답변처이기 때문이다.

**결과**:
- `app/privacy/page.tsx` +29/−1, tsc·lint 통과, 사용자 육안 확인 완료
