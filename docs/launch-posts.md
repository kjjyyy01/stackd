# 런칭 포스트 초안 3종 (Day 18~19 · 런칭·게시 **9/10 목**, 배포는 9/8 선행)

> ⏰ **런칭·게시일 변경 (2026-09-08, 사용자 결정)**: ~~9/8~~ → **9/10(목)로 통일**. 배포(`be689d1`)는 9/8 선행 완료됐으나 공개 런칭일을 게시일과 같은 **9/10**로 잡는다 — 9/10에 게시하므로 본문 "오늘 런칭했습니다"는 그대로 맞다. 판정일 9/22→**9/24(목)**(런칭 9/10+14). **"8/19 이후 런칭일 불변" 소재는 삭제**(2026-09-08 사용자 판정 — 런칭 9/8→9/10로 무효화): 소재 블록·프로세스 문단·"3주 동안 있었던 일" 목록·재게시 후보에서 제거 완료. *(이전 기록: "9/8 오전"→"9/8 저녁"→"9/10 통일".)*
...
- 기간: 8/11(Day 0) → 9/10 런칭(배포 9/8 완료), 실작업일 20 + 버퍼 2. 일정 조정 4회·버퍼 소진 2회
...
AI 도구 쪽은 superpowers를 유일한 오케스트레이터로 두고, 기획은 make-plan → make-prd로 ID 스킴(SCR/BR/EVT…)을 박아서 구현 중 스코프가 조용히 흔들리는 걸 막았습니다. 구현 구간엔 YAGNI 강제(ponytail)를 상시로.
...
> ✅ **3번 글은 첫 게시에서 제외 확정 (2026-09-08, 사용자 결정)** — 표본 300명 미달 시 재게시(킬 크라이테리아 ③) 소재로 남긴다. 재게시 후보는 이 외에도: Safari 결함 3건 스토리 / 라이브러리에 올라온 다른 분 카드 소개.
