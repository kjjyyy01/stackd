...
> ⏰ **런칭·게시일 변경 (2026-09-10, 사용자 결정)**: ~~9/8~~ → **9/10(목)로 통일**. 배포(`be689d1`)는 9/8 선행 완료됐으나 공개 런칭일을 게시일과 같은 **9/10**로 잡는다 — 9/10에 게시하므로 본문 "오늘 런칭했습니다"는 그대로 맞다. 판정일 9/22→**9/24(목)**(런칭 9/10+14). ⚠️ **"8/19 이후 런칭일 불변" 소재는 이제 사실과 어긋남**(런칭 9/8→9/10) — 본문 20·54·89행·재게시 후보 3번 글에서 이 talking point 유지/삭제 판단 필요. *(이전 기록: "9/8 오전"→"9/8 저녁"→"9/10 통일".)*
...
> ✅ **3번 글은 첫 게시에서 제외 확정 (2026-09-08, 사용자 결정)** — 표본 300명 미달 시 재게시(킬 크라이테리아 ③) 소재로 남긴다. 재게시 후보는 이 외에도: Safari 결함 3건 스토리 / "런칭일이 한 번도 안 밀린 이유" / 라이브러리에 올라온 다른 분 카드 소개.
